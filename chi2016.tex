...
\begin{equation}
-C\sum_{s,a} \big[\min(0, Q^\ast(s,a)) - \max_{a^\prime \in A(s)\backslash a}Q^\ast(s,a^\prime)\big], \label{eqn:slack}
\end{equation}
...
\end{equation}
Q-learning tries to find the actio-value function satisfying Eq. \eqref{eqn:bellman}, by minimizing the squared difference $L$ between both sides of the equation. Let
\begin{equation*}
...
$$\frac{\partial L}{\partial \theta^i} = \mathbb{E} (Q^i(s_t,a_t)- r_{t} + \gamma\max_{a^\prime}Q(s_{t+1}, a^\prime | \theta^{i -}))\cdot \frac{\partial{Q^i(s_t,a_t)}}{\partial{\theta^i}},$$
